...
function setText( node, val ) {
	if ( typeof node.textContent === "string" ) {
		...
		throw new Error( "Don't know how to set text content" );
	}
}
		...
		"CLASS": function( className ) {
			...
			return pattern ||
				...
				classCache( className, function( elem ) {
					return pattern.test( typeof elem.className === "string" && elem.className || typeof elem.getAttribute !== sundefined && elem.getAttribute("class") || "" );
				});
		},
				...
				for ( i = 0; i < hasScripts; i++ ) {
					...
					if ( rscriptType.test( node.type || "" ) &&
						...
						if ( node.src ) {
							...
							DOMEval( Sizzle.getText( node ).replace( rcleanScript, "" ), doc );
						}
					}
				}
			}
